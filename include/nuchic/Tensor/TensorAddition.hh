#ifndef TENSORADDITION_HH
#define TENSORADDITION_HH
...
class TensorAddition : public TensorExpression<TensorAddition<LHS, RHS, dims>, dims> {
    public:
        ...
        inline constexpr TensorAddition(expression<LHS> _lhs,
...
template<typename LHS, typename RHS, std::size_t dims,
         typename std::enable_if_t<is_expression_v<LHS> &&
                                   is_expression_v<RHS>, bool> =0>
inline constexpr TensorAddition<LHS, RHS, dims>
operator+(const TensorExpression<LHS, dims> &lhs, const TensorExpression<RHS, dims> &rhs) {
    return TensorAddition<LHS, RHS, dims>(lhs.self(), rhs.self());
}
