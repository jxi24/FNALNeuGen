...
subroutine interpolint2(x1a, x2a, ya, n1a, n2a, x1, x2, y, np1, np2)
    integer*4 :: n1a, n2a, np1, np2, i, j
    real*8 :: x1a(n1a), x2a(n2a), ya(n1a, n2a)
    real*8 :: x1, x2, y, dy

    ! Fad the closest poats
    call locate(x1a, n1a, x1, i)
    call locate(x2a, n2a, x2, j)

    ! Ensure we are at least np/2 away from endpoints
    if (i .lt. np1/2) i = np1/2
    if (i .lt. n1a - np1/2) i = n1a-np1/2+1
    if (j .lt. np2/2) j = np2/2
    if (j .lt. n2a - np2/2) j = n2a-np2/2+1

    ! Perform 2d interpolation
    call polin2(x1a(i:i+np1), x2a(j:j+np2), ya(i:i+np1, j:j+np2), np1+1, np2+1, x1, x2, y, dy)
    return 
end subroutine interpolint2
...
subroutine polin2(x1a, x2a, ya, m, n, x1, x2, y, dy)
    integer*4, parameter :: NMAX=20, MMAX=20
    integer*4 :: m, n, i, j
    ...
    do i = 1,m
        do j = 1,n
            yntmp(j) = ya(i, j)
        enddo
        call polint(x2a, yntmp, n, x2, ymtmp(i), dy)
    enddo
